...
// ============================================================
//  LANGUAGE FILTER
//  Censors matched words to the same length with asterisks.
//  Catches basic evasion: 1337speak (@ 4 3 1 0 $), repeated
//  letters (fuuuck), and spaced-out letters (f u c k).
// ============================================================
const BAD_WORDS = [
  'fuck', 'shit', 'bitch', 'bastard', 'asshole', 'ass', 'dick', 'piss',
  'cunt', 'cock', 'pussy', 'whore', 'slut', 'douche', 'dumbass', 'jackass',
  'crap', 'damn', 'goddamn', 'hell', 'bollocks', 'bugger', 'wanker',
  'twat', 'prick', 'tosser', 'arse', 'nigger', 'nigga', 'faggot', 'fag',
  'retard', 'retarded', 'spic', 'chink', 'gook', 'kike', 'tranny',
  'cripple', 'whorebag', 'skank', 'slag', 'motherfucker', 'fucker',
  'fucking', 'shithead', 'shitty', 'bullshit', 'horseshit', 'dipshit',
  'clusterfuck', 'dumbfuck', 'cocksucker', 'asswipe', 'shitface', 'twit',
];

// Build one regex that matches a bad word even if separated by spaces/
// punctuation between every letter (e.g. "f.u.c.k" or "f u c k")
function toEvasionPattern(word) {
  const leet = { a: '[a@4]', e: '[e3]', i: '[i1!]', o: '[o0]', s: '[s$5]', u: '[uv]' };
  return word
    .split('')
    .map((ch) => {
      const cls = leet[ch] || ch;
      // allow the letter to repeat (fuuuck) and be followed by stray separators
      return `${cls}+[\\s\\-_.*]*`;
    })
    .join('');
}
...
const FILTER_REGEX = new RegExp(
  '\\b(' + BAD_WORDS.map(toEvasionPattern).join('|') + ')\\b',
  'gi'
);

function filterProfanity(text) {
  return text.replace(FILTER_REGEX, (match) => {
    // keep the first character, mask the rest — preserves roughly
    // where a word was without being fully unreadable. Preserve any
    // trailing whitespace the greedy match swallowed.
    const trailingWs = (match.match(/\s+$/) || [''])[0];
    const trimmed = match.slice(0, match.length - trailingWs.length);
    return trimmed[0] + '*'.repeat(Math.max(trimmed.length - 1, 1)) + trailingWs;
  });
}
...
const dmHistory = new Map(Object.entries(loadJson(DMS_FILE, {}))); // "userA|userB" (sorted) -> array of dm messages
...
function userList() {
  return Array.from(new Set(Array.from(clients.values()).map((c) => c.username)));
}
...
wss.on('connection', (ws) => {
  ws.on('message', (raw) => {
    ...
    if (data.type === 'admin_login') {
      ...
      if (code === ADMIN_CODE) {
        client.isAdminSession = true;
        ws.send(JSON.stringify({ type: 'admin_login_result', success: true }));
      } else {
        ws.send(JSON.stringify({ type: 'admin_login_result', success: false, error: 'Wrong admin code' }));
      }
      return;
    }
    ...
    if (data.type === 'kick') {
      const client = clients.get(ws);
      if (!client || !client.isAdminSession) return; // only unlocked admin sessions can kick
      const target = (data.target || '').toString().trim();
      if (!target || target === client.username) return;

      let kickedAnyone = false;
      for (const [sock, info] of clients.entries()) {
        if (info.username === target) {
          sock.wasKicked = true; // suppress the generic "left the chat" message below
          sock.send(JSON.stringify({ type: 'kicked', by: client.username }));
          sock.close();
          kickedAnyone = true;
        }
      }
